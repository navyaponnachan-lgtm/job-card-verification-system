...
**Job Card ↔ Account Book Verification System** — a desktop app that reads
handwritten job cards and ledger pages with AI, then automatically checks
whether the numbers actually match.

![LedgerLens screenshot](screenshot.png)

## What it does

Small businesses that still use handwritten job cards (service tickets)
and handwritten account books (ledgers) have to manually check that a job
card's billed total matches what actually got logged in the ledger.
LedgerLens automates that check:

1. Add photos of job cards and account book pages (drag-and-drop or file
   picker).
2. Google Gemini's vision AI reads the handwriting on both.
3. LedgerLens sums every ledger line for a job card's serial number and
   compares it to that job card's stated total — a serial can legitimately
   appear on more than one ledger line, so it's a sum, not a lookup.
4. Each job card gets one of four verdicts: **Verified**, **Amount
   Mismatch**, **Missing Record**, or **Manual Review** (used whenever the
   AI isn't confident enough to trust automatically).

## Download

Grab the latest Windows installer from
**[Releases](https://github.com/navyaponnachan-lgtm/job-card-verification-system/releases/latest)**
— download `LedgerLensSetup.exe`, run it, and follow the installer (Start
Menu shortcut + uninstaller included). No Python required.

You'll need a free Gemini API key
([get one here](https://aistudio.google.com/app/apikey)) — LedgerLens asks
for it the first time you run a verification, and remembers it after that.
...
- Drag-and-drop image upload with thumbnail previews
- Batch processing with a Stop button to cancel a run early
- A 3-run cross-check on account book pages — a serial's total is only
  trusted if 3 independent AI reads agree, since a single pass on dense
  handwritten ledgers isn't reliable enough (see the build log below for
  the real bug this fixes)
- Run history (SQLite), Excel/PDF export, live dashboard stats

## Project docs

- [PROJECT_SCOPE.md](PROJECT_SCOPE.md) — problem statement, architecture,
  field schema, and scope decisions
- The rest of this file is a dated build log of everything built, in
  order, kept for reference — updated after every meaningful change
